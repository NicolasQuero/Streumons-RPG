...
Entity::Entity(char type, int x, int y, int hp, int dmg) : type(type), pos(x, y), hp(hp), dmg(dmg), is_alive(true), attackInUse(' ') {}

Entity::Entity(char type, Pos p, int hp, int dmg) : type(type), pos(p), hp(hp), dmg(dmg), is_alive(true), attackInUse(' ') {}
